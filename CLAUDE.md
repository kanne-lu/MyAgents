...
| Cron 结果未投递到 IM | 检查 `deliver_cron_result_to_bot` 日志（`[CronTask]`）；确认 system-event POST 使用 `local_http` |
| Heartbeat 收到普通心跳而非 Cron 结果 | `systemEventQueue` 为空 → system-event 投递失败（同上 502 排查）或被上一次 heartbeat 已 drain |

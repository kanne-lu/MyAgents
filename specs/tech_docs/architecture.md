...
**Tauri Commands**：`cmd_start_im_bot`、`cmd_stop_im_bot`、`cmd_im_bot_status`、`cmd_im_all_bots_status`、`cmd_im_conversations`、`cmd_install_openclaw_plugin`、`cmd_uninstall_openclaw_plugin`、`cmd_list_openclaw_plugins`

**前端**：`src/renderer/components/ImSettings/` — 列表页 / 创建向导 / 平台选择 / OpenClaw 安装向导 / 详情页，通过 `useConfig()` 的 `refreshConfig()` 同步 React 状态。

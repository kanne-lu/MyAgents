...
Project (工作区)
  ...
  + 可选的「主动 Agent」模式 → AgentConfig（24h 感知与行动）
    └── Channels: Telegram / Feishu / Dingtalk / OpenClaw Plugin
```

**Rust 层**通过 `SidecarOwner::Agent(session_key)` 复用 SidecarManager 获取 Bun Sidecar。

```rust
/// v0.1.41+ Agent 管理容器
pub type ManagedAgents = Arc<RwLock<HashMap<String, AgentInstance>>>;
/// Legacy IM Bot 容器（向后兼容，逐步淘汰）
...
| `FeishuAdapter` | 飞书 WebSocket 连接、交互卡片、ACK 机制 |
| `DingtalkAdapter` | 钉钉 Stream 连接、消息收发 |
| `BridgeAdapter` | OpenClaw 社区插件桥接（Bun Plugin Bridge 进程代理收发） |
| `SessionRouter` | peer → Sidecar 映射，`SidecarOwner::Agent(session_key)` 管理生命周期 |
| `HealthManager` | 每 5s 持久化状态到 `~/.myagents/im_bots/{botId}/state.json` |
| `MessageBuffer` | Sidecar 不可用时缓冲消息，恢复后重放 |

**适配器模型**：`AnyAdapter` 枚举统一分派 `ImAdapter` + `ImStreamAdapter` trait 方法到 Telegram / Feishu / Dingtalk / Bridge 四种后端。社区插件通过 `BridgeAdapter` 接入——Rust 启动独立 Bun 进程加载 OpenClaw Channel Plugin，消息通过 HTTP 双向转发，AI 推理仍走 Rust → Bun Sidecar 标准管道。

**Tauri Commands（v0.1.41+）**：`cmd_start_agent_channel`、`cmd_stop_agent_channel`、`cmd_agent_channel_status`、`cmd_all_agents_status`、`cmd_update_agent_config`、`cmd_im_conversations`、`cmd_install_openclaw_plugin`、`cmd_uninstall_openclaw_plugin`、`cmd_list_openclaw_plugins`

> 旧命令 `cmd_start_im_bot` 等已标 `@deprecated`，内部转发到新 Agent API。

**前端**：`src/renderer/components/AgentSettings/` — Agent 通用设置 / Channel 管理 / 心跳配置 / 定时任务列表，通过 `useConfig()` 的 `refreshConfig()` 同步 React 状态。

详见 [IM 集成技术架构](./im_integration_architecture.md) 和 [Agent 架构 PRD](../prd/prd_0.1.41_agent_architecture.md)。
